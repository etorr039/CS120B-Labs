...
unsigned char i; // LED movement back and forth
unsigned char LCDcount; // score display
...
void lightGame() {
	...
	switch(state) { // actions
		...
		case reset:
			if (LED == 0x02 && LCDcount < 9) {
				LCDcount++;
			}
			else if ((LED == 0x01 || LED == 0x04) && LCDcount > 0) {
				LCDcount--;
			}
			if (LED == 0x02 && LCDcount == 9) { // WINNER!
				LCD_ClearScreen();
				LCD_DisplayString(17, "1v1 me scrub");
				state = start;
			}
			if ((LED == 0x01 || LED == 0x04) && LCDcount == 0) { // LOSER!
				LCD_ClearScreen();
				LCD_DisplayString(17, "u a bot");
				state = start;
			}
			break;
	}
...
int main(void) {
	...
	while (1) {
		...
		LCD_WriteData(LCDcount + '0'); // write display number to LCD
