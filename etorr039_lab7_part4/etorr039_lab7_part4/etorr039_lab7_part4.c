...
static unsigned long periodSpeaker = 1; // Task 4 period
const unsigned long periodFreq = 200; // Task 5 period
...
enum freqStates {Start5, wait, freqUp, freqDown}; // Task 5
...
int TickFct_freq(int state) {
	switch(state) { // transitions
		...
		case wait:
		if (BUTTON == 0x01 || BUTTON == 0x05) {
			state = freqUp;
		}
		else if (BUTTON == 0x02 || BUTTON == 0x06) {
			state = freqDown;
		}
		else {
			state = wait;
