 ...
 *	Lab Section: B21
 *	Assignment: Custom Lab hit detection 2 
...
enum win_States {playing, win, nextLVL};
...
int main(void)
{
	...
	tasks[j].period = 300; // state period
	tasks[j].elapsedTime = tasks[j].period;
	...
	j++;
	tasks[j].state = playing; // initial state
	tasks[j].period = 1000; // state period
	tasks[j].elapsedTime = tasks[j].period;
